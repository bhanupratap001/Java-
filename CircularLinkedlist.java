  ...
  class Linkedlist//Liked List class declartion
{
	...
	//if(size<5)//size is defined because it is cicular  a linkedList and temp.next can never find a NULL value
  //{
			...
			if(head==null)//here check LinkedList is alredy exist or not
			  
					{
						...
						else//if head is not null than execute this part
							{
								...
								while(temp.next!=head)//check while temp not equals to null
								{  
								  temp=temp.next;
				                     	
								//System.out.println("while loop"+temp.data);
			...
			{//size=0;
				if(head==null)
			   {
				   System.out.println("LinkedList not have any Node to travese");
				        ...
				        while(temp.next!=head)//because it a Circular linkedList so temp pointer never found null
				           {                
							
					          System.out.println("\nYour  Node  Data is\t"+temp.data);
					           temp=temp.next;
							   //size++;
							   if(temp.next==head){System.out.println("yes it is circularLinkedList");}
						   }
					}
			}
...
public static void main(String arg[])
                  {  
				     ...
				     li.addNode(70);	
                     //li.addNode(40);
                     //li.addNode(90);//this node can not be inserted because size is alredy defined that is 5
					 //li.addNode(80);//this node can not be inserted because size is alredy defined that is 5
